	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if i.State != tc.expState {
				t.Errorf("Expected state %d, got %d", tc.expState, i.State)
			}

			if i.ActualDuration != tc.expDuration {
				t.Errorf("Expected duration %s, got %d", tc.expDuration, i.ActualDuration)
			}
			cancel()
		})
	}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			start := func(i pomodoro.Interval) {
				...
				if i.ActualDuration >= i.PlannedDuration {
					t.Errorf("EXpected duration %s, got %d.\n", i.ActualDuration, i.PlannedDuration)
				}
			}
				...
				if tc.cancel {
					t.Errorf("End callabck should no be executed")
				}
			}
